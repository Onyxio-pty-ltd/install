...
PACKAGE_PLATFORM="$(cat PLATFORM 2>/dev/null || echo unknown)"
PACKAGE_POSTGRES_IMAGE="$(cat POSTGRES_IMAGE 2>/dev/null || echo postgres:15)"
HOST_PLATFORM="$(host_platform)"

if [ "$PACKAGE_PLATFORM" != "unknown" ] && [ "$HOST_PLATFORM" != unknown/* ] && [ "$PACKAGE_PLATFORM" != "$HOST_PLATFORM" ]; then
  echo "This package contains ${PACKAGE_PLATFORM} Docker images, but this server is ${HOST_PLATFORM}." >&2
  echo "Build a matching package, for example:" >&2
  echo "  ./deploy/build-package.sh ${VERSION} ${HOST_PLATFORM}" >&2
  exit 1
fi
